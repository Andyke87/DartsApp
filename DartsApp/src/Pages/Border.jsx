  ...
  useEffect(() => {
    ...
    ringGroups.forEach(({ data, scale, colors, stroke, titleMultiplier }, index) => {
      ...
      g.selectAll()
        ...
        .attr("stroke-width", 5)
        .attr("stroke", stroke)
        .on("click", (event, d) => handleFieldClick(d.data.value * titleMultiplier))
        .append("title")
        .text(d => `${d.data.value * titleMultiplier}`);

      if (index === 0) { // Index 0 verwijst naar de buitenste ring
          const textSize = 50; // Pas deze waarde aan op basis van je behoeften
